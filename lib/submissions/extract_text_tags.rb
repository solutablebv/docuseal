    ...
    module ExtractTextTags
        ...
        def find_nodes_for_range(text_nodes, start_pos, end_pos)
            ...
            text_nodes.each do |node|
                ...
                if node_start < end_pos && node_end > start_pos
                    tag_nodes << node
                end
        ...
        def build_preferences(attrs, field_type)
            ...
            # Format
            if attrs['format'].present?
                preferences['format'] = attrs['format']
            end

            # Font settings
            if attrs['font'].present?
                preferences['font'] = attrs['font']
            end
            if attrs['font_size'].present?
                preferences['font_size'] = parse_numeric(attrs['font_size'])
            end
            if attrs['font_type'].present?
                preferences['font_type'] = attrs['font_type']
            end

            # Color
            if attrs['color'].present?
                preferences['color'] = attrs['color']
            end

            # Alignment
            if attrs['align'].present?
                preferences['align'] = attrs['align']
            end
            if attrs['valign'].present?
                preferences['valign'] = attrs['valign']
            end

            # Dimensions (if specified)
            if attrs['width'].present?
                preferences['width'] = parse_numeric(attrs['width'])
            end
            if attrs['height'].present?
                preferences['height'] = parse_numeric(attrs['height'])
            end

            # Method for verification fields
            if field_type == 'verification' && attrs['method'].present?
                preferences['method'] = attrs['method']
            end

            # Mask
            if parse_boolean(attrs['mask'], false)
                preferences['mask'] = true
            end
